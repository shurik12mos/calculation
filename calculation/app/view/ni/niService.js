...
app.service('Ni', function () {
    ...
    this.prop.reserve = {
        ...
        calculate: function (sum, am, total) {
            var total = self.prop.total.value;
    ...
    this.prop.deliveryExp = {
        ...
        calculate: function (sum, am, total, totalMaterials) {
            ...
            this.value += totalMaterials * (1 + this.percent / 100) * (1 / (1 - self.prop.singleTax.percent / 100) - 1);
    ...
    this.prop.possibleBonus = {
        ...
        calculate: function (sum, am, total) {
            ...
            prom = (self.prop.promotion.ofTotal || 0) - 5;
            salary = prop.salaryMainWorkers.value + prop.salaryBrigadier.value;
            salary += prop.salaryManager.value + prop.salaryProject.value;
            this.value = self.prop.total.value * prom / salary;
            this.value = Math.round(this.value * 100) / 100;
        }
    };
    ...
    this.calc = function (sumSalary, amortization, total) {
        var key,
            sellerServicePercent,
            brigadierServicePercent;
        ...
        for (key in self.prop) {
            if (!self.prop.hasOwnProperty(key)) continue;
            if (self.prop[key].id > self.prop.total.id) continue;
            self.prop[key].ofTotal = Math.round(self.prop[key].value * 100 / self.prop.total.value * 100) / 100;
            self.prop[key].ofTotal = self.prop[key].ofTotal || 0;
        }

        sellerServicePercent = (self.prop.sellExp.value / self.prop.total.value);
        sellerServicePercent = sellerServicePercent - 0.05 * (1 - 1 * self.prop.singleTax.value - 0.8333 * self.prop.VAT.value);
        brigadierServicePercent = sellerServicePercent / 7;
        self.prop.sellExp.servicePercent = sellerServicePercent;
        self.prop.salaryBrigadier.servicePercent = brigadierServicePercent;
